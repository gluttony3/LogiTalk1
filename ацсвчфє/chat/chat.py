...
fon_img = Image.open("font/wallaper.jpg")
font_ready = CTkImage(light_image=fon_img, size=(800, 600))
background_font = CTkLabel(window, image=font_ready, text="")
background_font.place(y=0, x=0)
...
def send_message():
    ...
    chat_ready.insert(END, "Bogdan: " + message + "\n")
    chat_ready.configure(state="disabled")
...
threading.Thread(target=polyhenie, daemon=True).start()
